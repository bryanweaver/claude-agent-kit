...
/**
 * Validate source is within templates directory and destination is within .claude directory.
 * Resolves symlinks before validation to prevent symlink attacks.
 * @param {string} src - Source path
 * @param {string} dest - Destination path
 */
async function validateSourceAndDest(src, dest) {
  ...
  try {
    ...
    // If realpath fails (path doesn't exist yet), fall back to resolve
  ...
  if (relativeSrc.startsWith('..') || path.isAbsolute(relativeSrc)) {
    throw new Error('Security: Source must be within templates directory');
  }

  // Resolve symlinks for dest to prevent symlink escape attacks
  let absoluteDest;
  try {
    // Resolve the parent directory (which should exist) to catch symlinks
    const destParent = path.dirname(path.resolve(dest));
    const resolvedParent = await realpath(destParent);
    absoluteDest = path.join(resolvedParent, path.basename(dest));
  } catch {
    absoluteDest = path.resolve(dest);
  }
...
export async function copyFile(src, dest) {
  try {
    await validateSourceAndDest(src, dest);
