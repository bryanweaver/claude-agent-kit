...
/**
 * Create an empty stack detection result
 * @returns {Object} Empty stack with all fields set to null
 */
function createEmptyStack() {
  return { language: null, frontend: null, backend: null, database: null, testing: null, ui: null };
}
...
function detectFromPackageJson(projectPath) {
  ...
  // UI library detection (shadcn first since it implies tailwind)
  if (hasDependency(pkg, '@radix-ui/react-slot') || existsSync(path.join(projectPath, 'components.json'))) {
    ...
    result.ui = 'bootstrap';
  } else if (hasDependency(pkg, 'tailwindcss')) {
    result.ui = 'tailwind';
  }
