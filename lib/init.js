...
export async function init(options = {}) {
  ...
  // Step 6: Install tech-agnostic agents
  const templateDir = path.join(__dirname, '../templates');
  const techAgnosticAgents = ['shipper.md', 'reviewer.md', 'documentor.md', 'meta-agent.md', 'meta-commands-agent.md'];
  ...
  const commands = await listFiles(path.join(templateDir, 'commands'), '.md');
  for (const command of commands) {
    const src = path.join(templateDir, 'commands', command);
    const dest = path.join(targetDir, 'commands', command);
    await copyFile(src, dest);
  }
  spinner.succeed(`Installed ${commands.length} command(s)`);

  // Step 8: Install hooks (optional)
  spinner.start('Installing hooks...');
  const hooks = await listFiles(path.join(templateDir, 'hooks'), '.cjs');
  for (const hook of hooks) {
    const src = path.join(templateDir, 'hooks', hook);
    const dest = path.join(targetDir, 'hooks', hook);
    await copyFile(src, dest);
  }
  spinner.succeed(`Installed ${hooks.length} hook(s)`);

  // Step 9: Install skills if they exist for this stack
  try {
  ...
  console.log();
  console.log(chalk.gray('  Commands:'), chalk.white(`${commands.length} installed`));
  console.log(chalk.gray('    /ship, /fix, /cleanup, /test, /add-tests, and more'));

  console.log();
  console.log(chalk.blue('  Next steps:'));
  console.log(chalk.gray('    1. Restart Claude Code to load new agents'));
  console.log(chalk.gray('    2. Try /ship to start building features'));
  console.log(chalk.gray('    3. Use /create-agent to add custom agents'));
  console.log();
}
